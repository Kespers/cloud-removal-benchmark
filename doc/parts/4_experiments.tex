...
\begin{itemize}
    ...
    \item Tra le patch rimanenti, il metodo seleziona quella con la minore copertura nuvolosa usando \textit{s2cloudless} \cite{s2cloudless}.
\end{itemize}
...
Viene fatto uso di residual network (ResNet). Queste si basano sull'utilizzo di connessioni dirette, chiamate "scorciatoie" (shortcut), che saltano alcuni strati computazionali per trasportare le informazioni verso le parti inferiori della rete.
...
Non è stato possibile eseguire l'inferenza su servizi cloud come Google Colab. Il tempo di compilazione del piano gratuito era esaurito. 
...
L'obiettivo iniziale era processare l'intero dataset preso in esame (asiaWest). Tuttavia, la stima dei tempi superava le 40 ore. Il dataset di valutazione è stato quindi ridimensionato. L'analisi è stata limitata alle prime \textbf{50 patch}, per un totale di circa \textbf{1450 immagini}. Questa mole di dati ha comunque richiesto \textbf{oltre 6 ore} di elaborazione parallela.
...
Per ogni patch erano disponibili circa 29 immagini. È stata effettuata una selezione automatizzata per trovare il miglior risultato di rimozione nuvolosa. Tramite la libreria \texttt{s2cloudless}, è stata calcolata la copertura nuvolosa residua. L'immagine finale scelta è stata quella con la minore frazione di nuvole (\textit{Least Cloudy}).
